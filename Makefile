...
# $(foreach var,text,commond) 函数说明: 函数允许用户对文件列表中的每个文件执行特定的操作。
# 实例：text := a b c d; $(foreach n,$(names),$(n).o)=a.o b.o c.o d.o
# subst <from>,<to>,<text> 函数说明: 函数的功能是把字符串 text 中的 from 替换成 to，返回值为替换后的新字符串。实例：${subst *, %, *.h} = %.h
# filter <pattern>, <text> 函数说明：函数的功能是过滤出 text 中符合模式 pattern 的字符串，可以有多个 pattern 。返回值为过滤后的字符串。实例：${filter %.cpp %.o,  a.cpp b.o c.exe}=a.cpp b.o
...
#OBJS ?= $(wildcard *.cpp*)
# 多个目录
#OBJS ?= $(wildcard *.cpp* $(SRC)/*.cpp*)
OBJS ?= main.cpp
# 注意： 这里本来使用src配置源文件，objs配置目标文件如*.o 文件; 
# 原始makefile, src配置的为多项目编译处理，objs配置为主程序名称即：objs ?= main.cpp; 具体看程序makefile说明
